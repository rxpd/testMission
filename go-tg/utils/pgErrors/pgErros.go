...
func GetTerritoryForeignKeyViolation(ids []int)  {

}

func GetSQLSTATECode(err error) string {
	regex := regexp.MustCompile(`(SQLSTATE [0-9]+)`)
	regexResult := regex.FindString(err.Error())
	return strings.TrimLeft(strings.TrimRight(regexResult, ")"), "(SQLSTATE ")
}
